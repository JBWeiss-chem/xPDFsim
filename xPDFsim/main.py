...
def plot_R_r(df, rho, sigma, r_max, histogram):
    """Generates bokeh plot of R(r) functions.
    ...
    Args:
        ...
        histogram (Boolean): If True, no gaussian broadening is applied.

    Returns:
        dataframe: Dataframe containing broadened data.
    """
...
def plot_g_r(df, rho, sigma, r_max, histogram):
    """Generates bokeh plot of g(r) functions.

    Args:
        df (dataframe): Dataframe generated by gen_R_r().
        rho (float): Number density of the material.
        sigma (float): Standard deviation for Gaussian broadening.
        r_max (float or int): Maximum r to which the PDF was calculated
        histogram (Boolean): If True, no gaussian broadening is applied.
    ...
    # Plot full PDF
    p.line(radii_broad, g_r_total, color="black", line_width=1.5, 
           legend_label="g(r)")    
...
def plot_G_r(df, rho, sigma, r_max, histogram):
    """Generates bokeh plot of G(r) function.
    ...
    # Plot full PDF
    p.line(radii_broad, G_r_broad, color="black", line_width=1.5, 
           legend_label="G(r)")
...
def main(args):
    """This function gets executed when the xPDFsim command is run."""
    print(" ")
    try:
